...
create_window1 (void)
{
  ...
  for(i=0; i<7; ++i)
   {
   	for(j=0; j<7; ++j)
        ...
        if(pixbuf!=NULL) // pixbuf somewhat disappears on windows, hence I provide stock icon there instead
         ...
         board[i][j].img = gtk_image_new_from_stock(GTK_STOCK_YES,GTK_ICON_SIZE_LARGE_TOOLBAR);
        // DRAG AND DROP PART ********************************************************************
        ...
        if(pixbuf!=NULL) // pixbuf somewhat disappears on windows, hence I provide stock icon there instead
         ...
         gtk_drag_source_set_icon_stock(board[i][j].btn, "GTK_STOCK_YES");
        
        gtk_signal_connect((gpointer) board[i][j].btn, "drag_motion",
                           G_CALLBACK(DNDDragMotionCB), board[i][j].img);
        gtk_signal_connect((gpointer) board[i][j].btn, "drag_begin",
                           G_CALLBACK(DNDBeginCB), (gpointer) pixbuf);
        gtk_signal_connect((gpointer) board[i][j].btn, "drag_end",
                           G_CALLBACK(DNDEndCB), (gpointer) window1);
        gtk_signal_connect((gpointer)board[i][j].btn, "drag_data_get",
                           G_CALLBACK(DNDDataRequestCB), board[i][j].img);
        gtk_signal_connect((gpointer)board[i][j].btn, "drag_data_received",
                           G_CALLBACK(DNDDataRecievedCB), board[i][j].img);
        gtk_signal_connect((gpointer)board[i][j].btn, "drag_data_delete",
        ...
        gtk_widget_show(board[i][j].btn);
    	   gtk_table_attach(GTK_TABLE(table1),board[i][j].btn,j,j+1,i,i+1,
  ...
  btnReset = gtk_button_new_from_stock (GTK_STOCK_NEW);
